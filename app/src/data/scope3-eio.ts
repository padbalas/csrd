...
type FactorSet = {
  ...
  currency: string;
  source: string;
  categories: Array<Category & Scope3Factor>;
};
...
const buildCategories = (overrides: Record<string, Scope3Factor>) =>
  CATEGORY_DEFINITIONS.map((base) => ({ ...base, ...overrides[base.id] }));

export const SCOPE3_FACTOR_SETS: Record<string, FactorSet> = {
  US: {
    model: 'USEEIO v2.0',
